...
namespace CargaClic.Repository
{
    public class OrdenReciboRepository : IOrdenReciboRepository
    {
        ...
        public async Task<long> identifyDetail(OrdenReciboDetalleForIdentifyDto command)
        {
           ...
           if(cas != null)
           {
                cantidad_pallets  =   ( command.untQty / cas.UntQty  ) / pallet.UntQty ; // Cantidad de pallets
                   #region Calculo de cantidad de paletas *** #palletunitarios ***
              
                if(cantidad_pallets > 0)
                { 
                    r=  command.untQty % (cas.UntQty * pallet.UntQty)  ; 
                    for (int i = 0; i < cantidad_pallets; i++)
                    {
                        pallets.Add(cas.UntQty * pallet.UntQty);
                    }
                    if(r > 0) 
                    pallets.Add(r);
                }
        ...
        public async Task<IEnumerable<GuiaCabeceraDto>> ListarGuiaCabecera(int? idcliente, string fecharegistro)
        {
            var parametros = new DynamicParameters();
            // parametros.Add("idordentransporte", dbType: DbType.String, direction: ParameterDirection.Input, value: idordentrabajo);
            using (IDbConnection conn = Connection)
            {
                string sQuery = "[recepcion].[pa_listarcaberasguias]";
                var result = await conn.QueryAsync<GuiaCabeceraDto>(sQuery,
                                                                           parametros
                                                                          ,commandType:CommandType.StoredProcedure
                  );
                return result;
        ...
        public async Task<IEnumerable<GuiaDetalleDto>> ListarGuiaDetalle(long idcabecera)
        {
             var parametros = new DynamicParameters();
             parametros.Add("idcabecera", dbType: DbType.Int64, direction: ParameterDirection.Input, value: idcabecera);
            using (IDbConnection conn = Connection)
            {
                string sQuery = "[recepcion].[pa_listarguiasdetalle]";
                var result = await conn.QueryAsync<GuiaDetalleDto>(sQuery,
                                                                           parametros
                                                                          ,commandType:CommandType.StoredProcedure
                  );
        ...
        public async Task<IEnumerable<GuiaCabeceraDto>> ListarGuiasPendientes(int? idcliente, string fecharegistro)
        {
              var parametros = new DynamicParameters();
            // parametros.Add("idordentransporte", dbType: DbType.String, direction: ParameterDirection.Input, value: idordentrabajo);
            using (IDbConnection conn = Connection)
            {
                string sQuery = "[recepcion].[pa_listarguiaspendientes]";
                var result = await conn.QueryAsync<GuiaCabeceraDto>(sQuery,
                                                                           parametros
                                                                          ,commandType:CommandType.StoredProcedure
                  );
