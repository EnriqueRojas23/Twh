...
namespace CargaClic.API.Controllers.Despacho
{
    ...
    public class OrdenSalidaController : ControllerBase
    {
        ...
        public OrdenSalidaController(IConfiguration config,
         ...
         IMapper mapper, IRepository<ErrorGuia> repositoryErrorGuia, IRepository<Producto> repositoryProducto )
        {
      ...
      public async Task<IActionResult> UpdateOrdenSalida(OrdenSalidaForRegister ordenSalidaForRegister)
      {
        ...
        public async Task<IActionResult> UploadFile(int usrid)
        {
          // var seguimiento = new Seguimiento();
            ...
            OrdenSalidaForRegister o = new OrdenSalidaForRegister();
            ...
            foreach (var item in entidades)
            {
                ...
                if(agregados.Where(x=> x == item.pedido).SingleOrDefault() != null )
                continue;
                   
                var cliente = await _repositoryCliente.GetAll(x=>x.Nombre  == item.razonsocial);
                if(cliente.Count() > 0)
                {
                    int i = 0;
                }
                ...
                bloque = item.pedido;
                agregados.Add(bloque);

              var direccion = await _repositoryDireccion.GetAll(x=> x.direccion == item.direccion  );
               if(direccion.Count() == 0)
                 {
                    odireccion = new Direccion();
                    odireccion.direccion = item.direccion;
                    odireccion.Clienteid = cliente.First().Id;
                    odireccion.Activo = true;
                    odireccion.codigo = "a";
                    odireccion.principal= true;
                    odireccion.iddistrito = 1;

                    await _repositoryDireccion.AddAsync(odireccion);
                    await _repositoryDireccion.SaveAll();
              }

                 direccion = await _repositoryDireccion.GetAll(x=> x.direccion == item.direccion  );


                o = new OrdenSalidaForRegister();
                o.AlmacenId = 8;
                o.ClienteId = cliente.First().Id;
                o.NumOrden = "";
                o.OrdenCompraCliente = "";
                o.PropietarioId = 47;
                o.UsuarioRegistro = 1;
                o.DireccionId = direccion.ToList()[0].iddireccion;
                o.GuiaRemision = bloque;
                o.FechaRegistro = "10/11/2021";
                o.FechaRequerida = "10/11/2021";
                o.HoraRequerida = "03:00";
                o.Propietario = "Eulen";


                var createdUser = await _repo_OrdenSalida.RegisterOrdenSalida(o);

                OrdenSalidaDetalleForRegister odet = new OrdenSalidaDetalleForRegister();

               var entidades2 = entidades.Where(x=> x.pedido == bloque);

              foreach (var item2 in entidades2)
              {
                    var idproducto =await  _repositoryProducto.Get( x => x.Codigo == item2.codigo );

                    odet.ProductoId = idproducto.Id;
                    odet.Cantidad = item2.stock.Value;
                    odet.EstadoID = 7;
                    odet.HuellaId =1 ;
                    odet.OrdenSalidaId = createdUser;
                    

              

                    var resp = await _repo_OrdenSalida.RegisterOrdenSalidaDetalle(odet);
         ...
         public List<CargaMasivaDetalleForRegister> ObtenerEntidades_CargaMasiva(List<List<String>> data) 
        {
                ...
                foreach (var item in data.Skip(1))
                {
                    ...
                    linea.razonsocial = item[6] ;
                    linea.codigo = item[8] ;
                    linea.pedido = item[0] ;
                    linea.stock =  Convert.ToInt32(item[10]);
                    linea.direccion = item[4];
                    linea.codigodespacho = item[11];
                    linea.contacto = item[12];
                    linea.telefono = item[13];
                    linea.distrito = item[14];
                    linea.departamento = item[15];
